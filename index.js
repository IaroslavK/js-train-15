...
function generateRandomPassword(length) {
    ...
    for (let i = 0; i < length; i++) {
        ...
        const symbol = characters[randomIndex];
        password += symbol;
    }
    return password;
...
function roundObjectValues(obj) {
    ...
    const map = arr.map(([key, value], index) => {
...
function calculateVolumeCylinder(radius, height) {
    ...
    return Math.ceil(cylinderVolume);
...
function getFractionalPart(num) {
    ...
    const double = Math.fround(num - integer);
    return double;
...
function estimateInvestment(principal, interestRate, years) {
    ...
    const amountOfInvestment = principal * Math.pow(1 + interestRate, years);
    return Math.round(amountOfInvestment);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
    ...
    const totalPrice = products.reduceRight((acc, val) => acc + val.price, 0);

    return Math.fround(maxPrice) <= Math.fround(totalPrice);
